...
const server = setupServer(
	http.get(`${API}/books`, () => HttpResponse.json({ books: [mockBook], total: 1 })),
	http.get(`${API}/books/b1`, () => HttpResponse.json(mockBook)),
	http.post(`${API}/books`, () => HttpResponse.json(mockBook, { status: 201 })),
	http.patch(`${API}/books/b1`, () => HttpResponse.json({ ...mockBook, isbn: "000" })),
	http.delete(`${API}/books/b1`, () => HttpResponse.json({ message: "Deleted" })),
	http.get(`${API}/book-categories`, () => HttpResponse.json({ categories: [] })),
	http.get(`${API}/academic-papers`, () => HttpResponse.json({ papers: [mockPaper], total: 1 })),
	http.get(`${API}/academic-papers/pp1`, () => HttpResponse.json(mockPaper)),
	http.post(`${API}/academic-papers`, () => HttpResponse.json(mockPaper, { status: 201 })),
	http.patch(`${API}/academic-papers/pp1`, () => HttpResponse.json({ ...mockPaper, published_year: "2024" })),
	http.delete(`${API}/academic-papers/pp1`, () => HttpResponse.json({ message: "Deleted" })),
	http.get(`${API}/academic-paper-categories`, () => HttpResponse.json({ categories: [] })),
)
