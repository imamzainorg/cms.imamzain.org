...
const server = setupServer(
	http.post(`${API}/media/upload-url`, () => HttpResponse.json({ uploadUrl: R2_URL, key: "media/abc.jpg" }, { status: 201 })),
	http.put(R2_URL, () => new HttpResponse(null, { status: 200 })),
	http.post(`${API}/media/confirm`, () => HttpResponse.json(mockMedia, { status: 201 })),
	http.get(`${API}/media`, () => HttpResponse.json({ media: [mockMedia], total: 1 })),
	http.delete(`${API}/media/m1`, () => HttpResponse.json({ message: "Deleted" })),
)
