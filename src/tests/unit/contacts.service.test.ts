...
const server = setupServer(
	http.get(`${API}/forms/contacts`, () => HttpResponse.json({ contacts: [mockContact], total: 1 })),
	http.patch(`${API}/forms/contacts/c1`, async ({ request }) => {
		const body = await request.json() as { status: string }
		return HttpResponse.json({ ...mockContact, status: body.status })
	}),
	http.delete(`${API}/forms/contacts/c1`, () => HttpResponse.json({ message: "Deleted" })),
)
