...
const mockPost = { id: "p1", category_id: "cat1", cover_image_id: null, is_published: false, published_at: null, views: 0, created_at: "2024-01-01T00:00:00Z", updated_at: "2024-01-01T00:00:00Z", translations: [{ lang: "en", title: "Test Post", summary: null, body: "Content", slug: "test-post", is_default: true }] }

const server = setupServer(
	http.get(`${API}/posts/admin`, () => HttpResponse.json(wrap({ posts: [mockPost], total: 1 }))),
	http.get(`${API}/posts/p1`, () => HttpResponse.json(wrap(mockPost))),
	...
	http.get(`${API}/post-categories`, () => HttpResponse.json(wrap({ categories: [{ id: "cat1", translations: [{ lang: "en", title: "News", slug: "news", description: null }] }] }))),
)
...
describe("postsService", () => {
	...
	it("creates a post", async () => {
		const { data } = await postsService.create({ category_id: "cat1", translations: [{ lang: "en", title: "Test Post", body: "Content", slug: "test-post", is_default: true }] })
		expect(data.id).toBe("p1")
	})
